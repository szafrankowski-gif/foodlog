# CLAUDE.md — foodlog 開発規約

このリポジトリは個人用の食事・トレーニングログPWA。設計判断はclaude.aiのプロジェクトスレッド（チーム会議形式）で行い、ここには実装規約のみを置く。**このファイルにも、リポジトリにも、個人の健康データ・氏名・数値を絶対に書かない**（リポジトリは公開。データの正本はユーザーの秘密Gist）。

## アーキテクチャ
- **1種類1ファイル・最新版のみ**：app.js（全ロジック）/ index.html（構造とCSS）/ sw.js / manifest.json。ビルドなし・依存なし・フレームワークなし。この構成を維持し、ファイル分割やnpm導入を提案しない。
- ホスティング：GitHub Pages。sw.jsのキャッシュ運用により、更新は「アプリを閉じて開き直す」で反映。
- データ：localStorage（`mealog:data`）＋GitHub秘密Gistへの自動同期（`foodlog-data.json`）。マージは日付キーごとに`_m`（更新エポックms）が新しい方を採用。**起動時pullが完了するまでpushしない**（syncReadyガード）— この順序を壊すと他端末のデータを消す。
- AI機能：Anthropic API直叩き。食事概算=Haiku、写真・評価（ばかお）・スクショ読取=Sonnet。APIキー・GHトークンはlocalStorageのみ。

## 設計原則（変更禁止。変更はチャット側のチーム会議決定が必要。現行はv3.1増量フェーズ＝2026-07-23決定、栄養目標はv4.2体重連動式＝2026-09-15決定）
1. **30秒ルール**：記録は1日30秒以内。入力を増やす機能は原則却下。自動化・写真・スクショ読取で解決する。
2. **実績ベース判定**：日区分（筋トレ日/高強度日/休養日）は宣言でなく実績から自動判定。筋トレ(trainA/B)は1種目以上チェックで筋トレ日。climb/jiujitsu/mountainはトグル＝実績で高強度日（目標は筋トレ日と同じ）。
3. **aerobicは日区分判定に影響させない**（糖質目標を変えない、週次カウントのみ）。増量フェーズでは維持レベル扱い：散歩・ウォーキングを運動実績として加点しない（記録・保存は従来どおり）。
4. **歩数は参考表示のみ**。目標・警告に使わない。唯一の例外：休養日15,000歩超で糖質+40〜50gの一言提案（目標値自体は不変）。
5. **カロリー・脂質は日次で数値管理しない**。消費カロリー・摂取カロリーの記録/表示/計算機能を追加しない（日次画面・メッセージ文言にkcal数値を出さない）。v4.2例外：脂質下限係数(f_floor)と参考エネルギーは月次判定の文脈でのみ扱う（日次表示は不可）。
6. **評価トーン**：称賛インフレ禁止・未達を責めない。増量フェーズは「食べる量を増やす」方向のみ助言し、減らす方向の文言を出さない（例外：月次判定の向かい風は天気フレーミングで伝える）。「食べすぎ」「食べ過ぎ」「オーバー」「失敗」「サボり」等の語をコードに含めない（grepで確認）。
7. 食材ペースは**月曜始まり・日曜締めの固定週**（WEEK_START=1）。
8. 食事時刻の優先順位：本文の時刻指定 > 写真EXIF(DateTimeOriginal) > lastModified > 今日なら現在時刻 > null。過去日の後入力に時刻を捏造しない。
9. **糖質は下限管理**（v3.1〜）：上限の概念なし。下限未達だけを扱い、超過を警告しない。目標値はv4.2で体重連動式（係数×体重7日平均、月曜更新）：係数のデフォルトはBULK_COEF相当のBULK_DEF、実効値はdata.bulk_config（Gist同期）に外出しし、機能側にハードコードしない。体重データ不足時のみGOALS固定値にフォールバック。判定は月次エンジン（機械判定→本人承認で係数±0.5g/kg/月まで）・伝達は気流1行（数字を出さない）・日常は沈黙。
10. **フローとストックを混ぜない**（v3.1〜）：日次フロー（①食事・睡眠・体重）/実施ログ（②筋トレ・moves）/身体能力ストック（③day.meas）は別性質。③は測定タブ専用、ホームは1行ティッカーのみ。測定リマインドは週1（日曜夜）だけ。

## データスキーマ（day オブジェクト）
`{ foods: [{name, p, c, veg, omega3, fiber, cat, t}], acts: ["trainA"|"trainB"|"climb"|"jiujitsu"|"mountain"|"aerobic"], actTimes: {}, workout: {checks:[], note}, moves: [{kind, min, t}], sleep, bedtime, waketime, weight, muscle, fatpct, rhr, steps, mood, wrist, creatine, vitd, comment, drill, meas: {gripR,gripL,kneeR,kneeL,boxR,boxL,hang,dips,dipsWide,dipsW,dipsWkg,pullup,pullupW,pullupWkg,swing,swing1Kg,swing1R,swing1L,bulgKg,tguKg,jump,waist}, measNote, lifts: [{ex, load, reps, sets, t}], _m }`
- 後方互換を壊さない。旧フィールド（dayType等）はload()で移行。新フィールドはnull安全に。
- 挙上（v4.1）：`lifts`のloadは文字列（"自重"/"KB16"/"+5"…）・数値換算はビュー側。種目マスタの初期値はコード（LIFT_DEF）、ユーザー追加分はlocalStorage `mealog:liftex`（端末ローカル・Gist同期外）。懸垂・ディップスの挙上記録はday.measの該当系列にも自動反映（二重入力させない）。
- 特別キー（v4.2）：`data.bulk_config = { coef, exit_lines, judgments[], exit_history[], _m }`（日付キーでない唯一のキー。_mでGist同期・マージは他キー同様）。日付キー前提のループは`isDayKey()`でガードすること。撤退線の署名者名はデータ内のみ（リポジトリに本名を書かない）。

## 変更時の必須手順
1. `node --check app.js` で構文確認
2. ロジック変更時は簡易テスト（純関数を抽出してnodeで検証）
3. UI変更時はjsdomスモークテスト（スマホ390px/PC1280pxの両方。900px境界で2カラム切替）
4. localStorageの既存データで壊れないか（移行コードの確認）
5. コミットメッセージは日本語可・変更理由を1行

## してはいけないこと
- 秘密GistのID・トークン・健康データをコードやコミットに含める
- localStorage/sessionStorage前提を壊す（このPWAはオフライン動作が必須）
- 確認なしにデータスキーマの破壊的変更・キー名変更
- 機能追加の独断（実装はここ、機能の採否はチャット側の審査）
