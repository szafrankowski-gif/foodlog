...
const BG_CFG_KEY = "mealog:bgcfg"; // 血糖予報の較正係数（個人値のためリポジトリに置かず、設定タブから端末内にのみ保存）
...
重要——重量と栄養素を混同しない：
- 品名に添えられたg数（「白米258g」「鶏むね200g」等）は食材の重さであり、栄養素の量ではない。重さが分かる場合は成分換算に使う（例：炊いた白米258g→c約95・p約6、鶏むね肉200g→p約44）。糖質gを食材の重さと同じ値にしない。
- そのままの値として優先してよいのは、栄養素名を伴う明記（「P30」「たんぱく質30g」「糖質50g」「C40」等）だけ。
...
// ---------- 血糖予報（実験的） ----------
// エンジンのみ実装。較正係数（個人の健康数値）はコードに置かず、設定タブで貼った端末内JSONから読む。
// 係数未登録なら機能ごと非表示＝実験期間中のOFFも係数削除で実現。
const BG_RISE = 50, BG_FALL = 130; // 応答形状の仮定：食事開始から50分でピーク→130分でベースへ
function bgCfg() {
  try {
    const c = JSON.parse(localStorage.getItem(BG_CFG_KEY) || "null");
    if (!c || !Array.isArray(c.mtype) || !c.mtype.length || !c.hour || !Array.isArray(c.pcorr) || c.pcorr.length < 3 ||
        !Array.isArray(c.sleep) || c.sleep.length < 3 || !c.ex || !c.base) return null;
    return c;
  } catch (e) { return null; }
}
function bgTmin(t) { const m = /^(\d{1,2}):(\d{2})$/.exec(t || ""); return m ? Number(m[1]) * 60 + Number(m[2]) : null; }
function bgMeals(cfg, day) {
  ...
  const meals = [...groups.values()].sort((a, b) => a.min - b.min).map((g) => {
    const C = g.foods.reduce((a, f) => a + (Number(f.c) || 0), 0);
    const P = g.foods.reduce((a, f) => a + (Number(f.p) || 0), 0);
    const q = g.foods.filter((f) => (Number(f.c) || 0) >= 5);
    const fc = q.length ? q.reduce((a, f) => a + typeCoef(f.name) * f.c, 0) / q.reduce((a, f) => a + f.c, 0) : 1;
    const r = C > 0 ? P / C : 1;
...
function renderLog() {
  ...
  return `
    ...
    ${(() => {
      ...
      const bg = bgCurveSvg(cfg, day);
      const col = !bg ? "var(--muted)" : bg.peak < 140 ? "var(--green)" : bg.peak <= 180 ? "var(--amber)" : "var(--red)";
...
function renderSettings() {
  ...
  return `
    <div class="section" style="padding-top:4px">
      ...
      <div class="card setbox">
        <div class="settitle">🩸 血糖予報（実験的）</div>
        <div class="setdesc">
          リブレ較正で得た<b>係数JSON</b>を貼ると、記録タブに1日の血糖予測カーブが表示されます。
          係数は個人の較正値のため<b>この端末の中にだけ</b>保存され、Gist同期にも含まれません（使う端末ごとに貼り付け）。
          削除すれば機能ごと非表示になります（リブレのベースライン測定中はOFF推奨）。
        </div>
        <div style="font-size:14px;margin-bottom:8px;color:${bgCfg() ? "var(--green)" : "var(--muted)"}">現在：${bgCfg() ? "係数登録済み ✓（予報ON）" : "未登録（予報OFF）"}</div>
        <textarea class="setinput mono" id="bgcfgInput" rows="3" placeholder='{"base":95,"mtype":[…],"hour":{…},"pcorr":[…],"sleep":[…],"ex":{…}}' style="font-size:12px"></textarea>
        <div class="setrow">
          <button class="setbtn" data-savebg>保存</button>
          ${bgCfg() ? `<button class="setbtn danger" data-delbg>削除（予報OFF）</button>` : ""}
        </div>
...
function bindEvents() {
  ...
  const sbg = $("[data-savebg]"); if (sbg) sbg.addEventListener("click", () => {
    ...
    try {
      ...
      if (!c || !Array.isArray(c.mtype) || !c.mtype.length || !c.hour || !Array.isArray(c.pcorr) || c.pcorr.length < 3 ||
          !Array.isArray(c.sleep) || c.sleep.length < 3 || !c.ex || !c.base) throw new Error("shape");
      localStorage.setItem(BG_CFG_KEY, JSON.stringify(c));
      setMsg = "血糖予報：係数を保存しました。記録タブにカーブが表示されます。";
    } catch (e) { setMsg = "血糖予報：JSONを読み取れませんでした。形式を確認してください。"; }
    render();
  });
  const dbg = $("[data-delbg]"); if (dbg) dbg.addEventListener("click", () => {
    if (confirm("血糖予報の係数を削除しますか？（予報は非表示になります。データ・記録は消えません）")) {
      localStorage.removeItem(BG_CFG_KEY); setMsg = "血糖予報：係数を削除しました（予報OFF）。"; render();
    }
  });
