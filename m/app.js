...
function protectSuggest(todayKey) {
  ...
  let hardDays = 0, noMoveDays = 0;
  for (let i = 0; i < 3; i++) {
    const d = new Date(); d.setDate(d.getDate() - i);
    const dd = data[toKey(d)] || {};
    if (dd.fatigue === "bad") hardDays++;
    if (!((dd.moves || []).some((m) => m.kind === "walk" || m.kind === "pilates"))) noMoveDays++;
  }
  return hardDays >= 2 || noMoveDays >= 3;
}
...
async function submitText() {
  ...
  for (const tok of tokens) {
    ...
    if (d) matched.push(d); else unknown.push(tok);
  }
  const key = toKey(cursor);
  const day = getDay(key);
  if (!unknown.length) {
    inputText = ""; errMsg = "";
    updateDay(key, { foods: day.foods.concat(matched.map((m) => Object.assign({ slot }, m))) });
    return;
  }
  if (!apiKey()) { errMsg = "辞書にない食品（" + unknown.join("・") + "）の概算にはAPIキーが必要です。設定タブから登録できます。"; render(); return; }
  busy = true; errMsg = ""; render();
  try {
    const items = await estimateItems(unknown);
    inputText = "";
    updateDay(key, { foods: getDay(key).foods.concat(matched.concat(items).map((m) => Object.assign({ slot }, m))) });
  } catch (e) {
...
function gaugeRow(icon, label, val, target, unit, praise, addNote, hideTarget) {
  ...
  const color = met ? "var(--green)" : "var(--blue)";
  return `
    <div class="grow">
      <div class="ghead">
      ...
      ${hideTarget ? "" : `<div class="hbar"><div class="fill" style="width:${pct}%;background:${color}"></div></div>`}
...
function renderLog() {
  ...
  const kcalNote = c.kcalFloor && K < c.kcalFloor
    ? `下限 ${Number(c.kcalFloor).toLocaleString()} まであと ${(c.kcalFloor - K).toLocaleString()} kcal。${c.kcalFloor - K <= 200 ? "ナッツひとつかみでちょうどです" : "足せると理想的です"}`
    : null;

  const subs = [
    ["🐟 魚", wk.fish, 3], ["🫘 大豆", wk.soy, 7], ["🥜 ナッツ", wk.nuts, 7],
    ["🫚 生姜", wk.ginger, 7], ["🧘 ピラティス", wk.pilates, 2], ["🚶 散歩", wk.walk, 5],
  ];
  ...
  return `
    ...
    ${guard ? `<div class="banner protect">🌸 今週は<b>守りの週</b>です。記録だけで十分。目標は気にしなくて大丈夫です。<button class="btn-s" data-endprotect style="margin-left:8px;height:26px">早めに終える</button></div>` : ""}
    ...
    ${obs ? `<div class="banner">🔍 観測期間 あと${obsDays}日。いつも通り食べてください。</div>` : ""}

    <div class="card g3">
      ${gaugeRow("🥩", "たんぱく質", P, c.pTarget ? Number(c.pTarget) : null, "g", "達成です。しっかり入りました", null, guard)}
      ${gaugeRow("🌾", "食物繊維", FI, c.fiberTarget ? Number(c.fiberTarget) : null, "g", "達成です", c.fiberTarget && FI < c.fiberTarget ? `あと ${(c.fiberTarget - FI).toFixed(1)}g 足せます（海藻・押し麦・オートミールが近道）` : null, guard)}
      ${c.sfiberTarget ? `<div class="gsub">└ うち水溶性 <b class="mono" style="color:${SF >= c.sfiberTarget ? "var(--green)" : "var(--text)"}">${SF}</b>${guard ? " g" : ` / ${c.sfiberTarget} g`}<div class="hbar"><div class="fill" style="width:${Math.min(SF / c.sfiberTarget, 1) * 100}%;background:${SF >= c.sfiberTarget ? "var(--green)" : "var(--blue)"}"></div></div></div>` : ""}
      <div class="grow">
        <div class="ghead">
    ...
    <div class="card wcard">
      ...
      ${c.wTarget ? `<span style="font-size:12.5px;color:var(--muted)">目標 ${c.wTarget} kg</span>` : ""}
      ${w7 ? `<span style="font-size:12.5px;color:var(--muted)">7日平均 <b class="mono">${w7}</b></span>` : ""}
      <span style="font-size:12.5px;color:var(--muted);margin-left:auto">体脂肪 <input class="mono" data-field="fatpct" inputmode="decimal" placeholder="—" value="${day.fatpct ?? ""}" style="width:56px"> %</span>
    </div>
    ${day.weight != null && day.weight !== "" && c.wFloor && Number(day.weight) < Number(c.wFloor)
      ? `<div class="wnote low">少し軽くなっています。今週は食べる量を意識してみましょう。</div>` : ""}
    ...
    <div class="stickybar">
      ...
      <div class="inputrow">
        <textarea class="mealinput" rows="2" placeholder="豆腐、蕎麦、鮭（単語だけでOK）" ${busy ? "disabled" : ""}>${esc(inputText)}</textarea>
        <button class="sendbtn" data-send ${busy ? "disabled" : ""}>${busy ? '<span class="spin">◐</span>' : "⏎"}</button>
      </div>
      <div class="hint">${busy ? "計算中…" : "いつもの食品は登録済みの量で即記録。区分（朝/昼/夜/間食）は上のチップで選べます。"}</div>
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-slot]").forEach((b) =>
    b.addEventListener("click", () => { slotSel = b.dataset.slot; render(); }));
